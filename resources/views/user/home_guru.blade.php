            ...
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    ...
                    <a class="dropdown-item" href="{{ route('logout') }}" data-toggle="modal" data-target="#logoutModal">
                        ...
                        <tbody>
                            @forelse($guru as $i => $guru)
                                <tr>
                                    <td>{{ ++$i }}</td>
                                    <?php
                                    $tmasuk=strtotime($guru->created_at);
                                    $tmasukf=date('l, j F Y', $tmasuk);
                                    $jmasuk=date('H:i', $tmasuk);
                                    ?>
                                    <td><?=$dhguru->created_at?></td>
                                    <td>{{ $guru->username }}</td>
                                    <td>
                                        <button type="button" class="btn btn-sm btn-info btn-circle" data-toggle="modal" data-target="#modal-detail{{$guru->id}}">
                                            <i class="fa fa-info" aria-hidden="true"></i> 
                                        </button>
                                    </td>
                                </tr>
                            @empty
                            <p>guru belum pernah masuk uks</p>
                            @endforelse
                        </tbody>
                        </table>
                </div>
